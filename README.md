...
Las cinco etiquetas interactivas del planeta se configuran en `config/site.php`, dentro de `network_services`: nombre, icono de Bootstrap Icons y descripción. `js/components/service-popovers.js` usa Bootstrap Popover y jQuery para abrir la información al pasar el cursor, enfocar con el teclado o pulsar. Un clic fija la información; otro clic en el botón, Escape o un clic fuera la cierra. Solo se muestra una ventana a la vez, y el cursor puede pasar a su contenido sin cerrarla. Los textos se escapan en PHP y se insertan como texto en nodos DOM. Los botones están fuera del elemento con `role="img"` y sus descripciones conservan un `title` nativo si no carga Bootstrap. El script se carga únicamente en el inicio.
